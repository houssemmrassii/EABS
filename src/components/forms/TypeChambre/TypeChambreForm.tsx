...
const TypeChambreForm = (props: Props) => {
  ...
  const onFinish = async (values: any) => {
    try {
      const data = await postTypeChambre(values);

      setDataSource([...dataSource, data]);
      resetAndClose();
      message.success("Le type chambre a été ajouté avec succès.");
    } catch (error) {
        ...
        items={[
          {
            key: "1",
            label: (
              <Typography.Text strong>Ajouter un Type Chambre</Typography.Text>
            ),
            children: (
              <Form
                layout="vertical"
